...
def data_generator(dataset_name, args, augment=False):
    ...
    valid_dataset = Load_Dataset(valid_dataset, args, augment)
    ...
    valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=args.batch_size,shuffle=True, drop_last=args.drop_last,num_workers=0)

    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=args.batch_size,
                                              shuffle=False, drop_last=False,num_workers=0)

    return train_loader, test_loader,trainAll_loader, valid_loader
...
def down_sampling(raw_data,max_len,multi_sampling_rates,indexes):
    ...
    max_len = len(index)
    #具体来说，我们使用torch.nn.upSample函数进行上采样操作，下采样
...
def up_sampling_generator(data_path,max_len,multi_sampling_rates,indexes,args, augment=False):
    ...
    valid_dataset['samples'] = up_sampling(valid_dataset['samples'],max_len,multi_sampling_rates,indexes)
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=args.drop_last, num_workers=0)
    val_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=args.batch_size,shuffle=False, drop_last=False, num_workers=0)
...
def down_sampling_generator(data_path,max_len,multi_sampling_rates,indexes,args, augment=False):
    ...
    valid_dataset['samples'],_ = down_sampling(valid_dataset['samples'],max_len,multi_sampling_rates,indexes)
    ...
    return train_loader, test_loader, val_loader, length
